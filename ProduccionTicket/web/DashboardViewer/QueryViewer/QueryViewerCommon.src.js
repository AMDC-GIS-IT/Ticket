...
qv.map = (function () {
					...
					if (qViewer.Chart.colorAxis.dataClasses.length != 0) {
						if (chart._coordSysMgr._coordinateSystems[0].regions[i].properties.center != undefined) {
							res.push({
								...
								value: data[j].value,
								itemStyle: {
									color: "#" + qViewer.Chart.Series.ByIndex[0].Points[j].Color.Color
								}
							});
						} else {
							res.push({
	...
	function mapaEchartsOptions(container, qViewer) {
		...
		function getLegendObject(qViewer, data) {
			...
			function legendSeriesBubble(qViewer, data, names) {
			...
			var series = legendSeriesBubble(qViewer, data, names);
		...
		if ((qViewer.MapType == QueryViewerMapType.Choropleth && qViewer.Chart.colorAxis.dataClasses.length != 0) ) { //color Scale
			...
			for (var i = 0; i < qViewer.Chart.colorAxis.dataClasses.length; i++) {
					...
					var splitList = {};
					var names = getLegendObject(qViewer,"")[0];
		...
		function getSeriesObject(qViewer, themeDefinitions) {
			...
			var series = {
				...
				emphasis: {
					...
					itemStyle: {
						areaColor: themeDefinitions.SelectionColor,
						opacity: 0.7
					}
				},
				select: {
			...
			else if (qViewer.MapType === QueryViewerMapType.Bubble) {
				var container = qViewer.getContainerControl();
		...
		if (qViewer.MapType == QueryViewerMapType.Bubble) {
			var data = convertData(getSerieData(qViewer), chart, qViewer);
			option = {
				color: getLegendObject(qViewer, data)[1],
				series: getLegendObject(qViewer, data)[2],
				legend: {
					orient: 'vertical',
					x: 'left',
					y: 'bottom',
					selectedMode: false,
					data: getLegendObject(qViewer, data)[0],
			...
			if (qViewer.MapType == QueryViewerMapType.Choropleth && gx.lang.gxBoolean(qViewer.ShowValues)) {
