...
namespace GeneXus.Programs {
   public class k2bscjoinstring : GXProcedure
   {
                                 ...
                                 out string aP2_Result )
      {
         ...
         ThreadPool.QueueUserWorkItem( PropagateCulture(new WaitCallback( executePrivateCatch )),objk2bscjoinstring);
         aP2_Result=this.AV12Result;
      }
      ...
      void executePrivate( )
      {
         ...
         AV12Result = "";
         AV15GXV1 = 1;
         while ( AV15GXV1 <= AV10CollectionString.Count )
         {
            AV11Item = AV10CollectionString.GetString(AV15GXV1);
            if ( AV8FirstItem )
            {
               AV12Result = AV11Item;
               AV8FirstItem = false;
