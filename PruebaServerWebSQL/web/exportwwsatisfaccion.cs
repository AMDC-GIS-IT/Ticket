...
namespace GeneXus.Programs {
   public class exportwwsatisfaccion : GXWebProcedure, System.Web.SessionState.IRequiresSessionState
   {
      public override void webExecute( )
      {
         ...
         if ( nGotPars == 0 )
         {
            ...
            if ( ! entryPointCalled )
            {
               AV14SatisfaccionFecha_From = context.localUtil.ParseDateParm( gxfirstwebparm);
               if ( StringUtil.StrCmp(gxfirstwebparm, "viewer") != 0 )
               {
                  AV15SatisfaccionFecha_To = context.localUtil.ParseDateParm( GetPar( "SatisfaccionFecha_To"));
                  AV17UsuarioFecha_From = context.localUtil.ParseDateParm( GetPar( "UsuarioFecha_From"));
                  AV18UsuarioFecha_To = context.localUtil.ParseDateParm( GetPar( "UsuarioFecha_To"));
                  AV19SatisfaccionResueltoNombre = GetPar( "SatisfaccionResueltoNombre");
                  AV20SatisfaccionTecnicoProblemaNombre = GetPar( "SatisfaccionTecnicoProblemaNombre");
                  AV21SatisfaccionTecnicoCompetenteNombre = GetPar( "SatisfaccionTecnicoCompetenteNombre");
                  AV22SatisfaccionTecnicoProfesionalismoNombre = GetPar( "SatisfaccionTecnicoProfesionalismoNombre");
                  AV23SatisfaccionTiempoNombre = GetPar( "SatisfaccionTiempoNombre");
                  AV24SatisfaccionAtencionNombre = GetPar( "SatisfaccionAtencionNombre");
                           ...
                           ref short aP12_OrderedBy )
      {
         ...
         initialize();
         executePrivate();
         aP12_OrderedBy=this.AV25OrderedBy;
      }

      public short executeUdp( DateTime aP0_SatisfaccionFecha_From ,
                               DateTime aP1_SatisfaccionFecha_To ,
                               DateTime aP2_UsuarioFecha_From ,
                               DateTime aP3_UsuarioFecha_To ,
                               string aP4_SatisfaccionResueltoNombre ,
      ...
      void executePrivate( )
      {
         ...
         if ( ! new k2bisauthorizedactivityname(context).executeUdp(  "Satisfaccion",  "Satisfaccion",  "List",  "",  "WWSatisfaccion") )
         {
            AV31Filename = "";
            AV32ErrorMessage = "You are not authorized to do activity";
            AV32ErrorMessage += "EntityName:" + "Satisfaccion";
            AV32ErrorMessage += "TransactionName:" + "Satisfaccion";
            AV32ErrorMessage += "ActivityType:" + "";
            AV32ErrorMessage += "PgmName:" + AV56Pgmname;
            AV33HttpResponse.AddString(AV32ErrorMessage);
            context.nUserReturn = 1;
            this.cleanup();
            if (true) return;
         }
         new k2bgetcontext(context ).execute( out  AV49Context) ;
         AV29Random = (int)(NumberUtil.Random( )*10000);
         AV31Filename = GxDirectory.TemporaryFilesPath + AV34File.Separator + "ExportWWSatisfaccion-" + StringUtil.Trim( StringUtil.Str( (decimal)(AV29Random), 8, 0)) + ".xlsx";
         ...
         {
            AV33HttpResponse.AppendHeader("Content-Type", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
         }
         if ( ! context.isAjaxRequest( ) )
         {
            AV33HttpResponse.AppendHeader("Content-Disposition", "attachment;filename=ExportWWSatisfaccion.xlsx");
         }
         if ( ! context.isAjaxRequest( ) )
         {
            AV33HttpResponse.AppendHeader("X-Frame-Options", "sameOrigin");
         }
         if ( ! context.isAjaxRequest( ) )
         {
            AV33HttpResponse.AppendHeader("X-Content-Type-Options", "nosniff");
         }
         AV33HttpResponse.AddFile(AV31Filename);
         new k2bremoveexceldocument(context).executeSubmit(  AV31Filename) ;
      ...
      public override void initialize( )
      {
         ...
         AV32ErrorMessage = "";
         AV56Pgmname = "";
         AV33HttpResponse = new GxHttpResponse( context);
         ...
         P003W2_A15UsuarioId = new short[1] ;
         ...
         AV56Pgmname = "ExportWWSatisfaccion";
         /* GeneXus formulas. */
         AV56Pgmname = "ExportWWSatisfaccion";
         context.Gx_err = 0;
      }
      ...
      private short[] P003W2_A15UsuarioId ;
      ...
      private GxHttpResponse AV33HttpResponse ;
